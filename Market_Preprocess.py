...
class Preprocessor(object):
    ...
    def run(self):
        ...
        print("SPY Plot Over Period")
        self.Plots().rollingAverage(SPY)
        print(self.ticker," Plot Over Period")
        self.Plots().rollingAverage(Stock)   
    ...
    def trainTestTransform(self,SPY,data,normalize=0):
        ...
        self.np.any(self.np.isnan(Index_Stock_Data))
        self.np.all(self.np.isfinite(Index_Stock_Data))
        
        Index_Stock_Data = Index_Stock_Data.replace([self.np.inf,-self.np.inf],self.np.nan)
